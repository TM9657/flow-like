...
export class DatabaseState implements IDatabaseState {
    ...
    async buildIndex(appId: string, tableName: string, column: string, indexType: IIndexType, optimize?: boolean): Promise<void> {
    ...
    async listItems(appId: string, tableName: string, offset?: number, limit?: number): Promise<any[]> {
         const isOffline = await this.backend.isOffline(appId);
    ...
    async queryItems(appId: string, tableName: string, query: IQueryTablePayload, offset?: number, limit?: number): Promise<any[]> {
         const isOffline = await this.backend.isOffline(appId);
