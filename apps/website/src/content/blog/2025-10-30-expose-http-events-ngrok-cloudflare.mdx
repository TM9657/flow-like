...
cover: "/images/expose-http-new.jpg"
draft: false
---
...
GitHub signs the exact request bytes. Validate the signature in an ingress that preserves those bytes before forwarding JSON to Flow-Like:
...
// Forward JSON.parse(rawBody.toString('utf8')) only after verification succeeds.
```
