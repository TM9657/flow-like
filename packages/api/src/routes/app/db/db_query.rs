...
use flow_like_storage::{databases::vector::{lancedb::{record_batches_to_vec, LanceDBVectorStore}, VectorStore}, datafusion::{self, prelude::SessionContext}};
...
pub async fn query_table(
    ...
    match (payload.vector_query, payload.fts_term, payload.filter) {
        (Some(vector_query), None, filter) => {
            ...
            let items = db.vector_search(vector_query.vector, filter_str, limit, offset).await?;
            return Ok(Json(items));
        }
        ...
        (Some(vector_query), Some(fts_term), filter) => {
            let filter_str = filter.as_deref();
            let items = db.hybrid_search(vector_query.vector, &fts_term, filter_str, limit, offset, payload.rerank.unwrap_or(true)).await?;
            return Ok(Json(items));
        }
        ...
        _ => {
            return Err(ApiError::BadRequest("No valid query parameters provided".to_string()));
        }
    }
}
