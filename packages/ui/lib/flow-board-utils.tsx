...
import { IPinType, type IPin } from "./schema/flow/pin";
...
export function doPinsMatch(
	...
	if (sourcePin.pin_type === targetPin.pin_type ) {
		console.warn(`Invalid connection: source and target pins have the same type (${sourcePin.pin_type})`);
		return false;
	}
            ...
            if (parentLayer !== currentLayer) {
                if (layer.id === currentLayer) {
                    ...
                    for (const pin of Object.values(layer.pins)) {
                        const inverted: IPin = { ...pin, pin_type: invertPinType(pin.pin_type) };
                    ...
                    nodes.push({
                        id: layer.id + "-input",
                        type: "layerInnerNode",
                        position: { x: layer.in_coordinates?.[0], y: layer.in_coordinates?.[1] },
                        zIndex: 19,
                        data: {
                            label: layer.id,
                            boardId: board.id,
                    ...
                    nodes.push({
                        id: layer.id + "-return",
                        type: "layerInnerNode",
                        position: { x: layer.out_coordinates?.[0], y: layer.out_coordinates?.[1] },
                        zIndex: 19,
                        data: {
                            label: layer.id,
                            boardId: board.id,
        ...
        for (const connectedTo of pin.connected_to) {
            ...
            const sourceNodeId =
                ((node as any)?.id ?? "") === (currentLayer ?? "")
                    ...
                    : (activeLayer.has(((node as any)?.layer ?? "")) ? (node as any).layer : (node as any)?.id);

            const targetNodeId =
                ((connectedNode as any)?.id ?? "") === (currentLayer ?? "")
                    ? resolveInnerNodeId(currentLayer!, conntectedPin)
                    : (activeLayer.has(((connectedNode as any)?.layer ?? "")) ? (connectedNode as any).layer : (connectedNode as any)?.id);

            if (pin.id && conntectedPin.id) edges.push({
