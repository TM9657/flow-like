...
export function useElementStorage(appId: string | undefined) {
	const pendingUpdates = useRef<Map<string, { value: unknown; timeout: NodeJS.Timeout }>>(new Map());

	// Store an element value (debounced to avoid excessive writes)
	const storeElementValue = useCallback((elementId: string, value: unknown) => {
		if (!appId) return;
	...
	const getElementValue = useCallback(async <T>(elementId: string): Promise<T | null> => {
	...
	const getAllElementValues = useCallback(async (): Promise<Record<string, unknown>> => {
		if (!appId) return {};
