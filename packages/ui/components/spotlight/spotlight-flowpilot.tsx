...
export function SpotlightFlowPilot({
	...
	return (
		<div className={cn("flex flex-col h-full", embedded && "min-h-[350px]")}>
			<div className="flex items-center gap-2 px-3 py-2 border-b border-border/40 shrink-0">
				...
				<div className="flex items-center gap-2">
					<div className={cn(
						...
						<Bot className={cn("text-violet-500", embedded ? "h-3.5 w-3.5" : "h-4 w-4")} />
					</div>
					<div>
						<h3 className={cn("font-semibold", embedded ? "text-xs" : "text-sm")}>FlowPilot</h3>
						{!embedded && <p className="text-[10px] text-muted-foreground">AI Assistant</p>}
					</div>
				</div>
			</div>

			<ScrollArea className={cn("flex-1 min-h-0", embedded ? "px-3" : "px-4")} ref={scrollRef}>
					...
					{messages.length === 0 && (
						...
						>
							<div className={cn(
								...
								<Sparkles className={cn("text-violet-500/60", embedded ? "h-5 w-5" : "h-8 w-8")} />
							</div>
							<h4 className={cn("font-medium mb-1", embedded ? "text-xs" : "text-sm")}>
								{embedded ? "Ask anything" : "How can I help?"}
							</h4>
							<p className={cn("text-muted-foreground", embedded ? "text-[10px] max-w-[180px]" : "text-xs max-w-[200px]")}>
								{embedded ? "I'm here to help with Flow-Like" : "Ask me about Flow-Like, workflows, or anything else!"}
							</p>
							<div className={cn("flex flex-wrap gap-1.5 justify-center", embedded ? "mt-3" : "mt-4 gap-2")}>
								{(embedded
									? ["Create flow", "What are nodes?", "Help"]
									: ["How do I create a flow?", "What are nodes?", "Help with storage"]
								).map((suggestion) => (
									<button
										key={suggestion}
										type="button"
										...
										className={cn(
											...
											embedded ? "text-[10px] px-2 py-1" : "text-xs px-3 py-1.5"
										)}
									>
										{suggestion}
									</button>
			...
			<div className={cn("border-t border-border/40 shrink-0", embedded ? "p-2" : "p-4")}>
				<div className="flex items-center gap-2">
					<input
						ref={inputRef}
						type="text"
					...
					>
						<SendHorizonal className={cn(embedded ? "h-3.5 w-3.5" : "h-4 w-4")} />
					</Button>
				</div>
				{!embedded && (
