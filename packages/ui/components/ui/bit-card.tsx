...
export function BitCard({
  ...
  const isProfileCompatible = ["Embedding", "ImageEmbedding", "Vlm", "Llm"].includes(bit.type);

  return (
    <div className={`${wide ? "md:col-span-2" : ""} group/card`} data-testid="box">
      <Card className={`
...
export function BitTypeIcon({
  ...
  const combinedClass =
    "min-h-4 min-w-4 h-4 w-4 text-foreground" + (className ? ` ${className}` : "");
