...
export class DownloadManager {
  ...
  constructor() {
    if (typeof window !== "undefined") {
      ...
      window.addEventListener("beforeunload", this.beforeUnloadBound, { once: true });
    }
  }
...
const createStore = () =>
  create<IDownloadManager>((set, get) => ({
    ...
    download: async (bit: IBit, cb?: (dl: Download) => void) => {
      ...
      if (!backend.bitState.downloadBit) throw new Error("Backend does not support downloading bits.");
...
export const useDownloadManager =
  (globalThis.__FL_DL_STORE__ ??= createStore());