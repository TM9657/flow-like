...
//         );
//         node.add_icon("/flow/icons/tree.svg");
...
//         node.add_output_pin("count", "Count", "Number of children", VariableType::Integer);
...
//                 .unwrap_or(&vec![])
//                 .iter()
//                 .filter_map(|p| super::parse_confluence_page(p, true))
//                 .collect::<Vec<_>>()
//         } else {
//             // Server v1 API
//             let mut url = format!(
...
//                 .unwrap_or(&vec![])
//                 .iter()
//                 .filter_map(|p| super::parse_confluence_page(p, false))
//                 .collect::<Vec<_>>()
//         };
...
//         node.add_output_pin("depth", "Depth", "Depth in page hierarchy", VariableType::Integer);
