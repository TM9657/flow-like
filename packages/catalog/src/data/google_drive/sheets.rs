...
impl NodeLogic for CreateGoogleSpreadsheetNode {
    async fn get_node(&self, _app_state: &FlowLikeState) -> Node {
        ...
        node.add_required_oauth_scopes(GOOGLE_PROVIDER_ID, vec!["https://www.googleapis.com/auth/spreadsheets"]);
        ...
        match response {
            Ok(resp) if resp.status().is_success() => {
                ...
                if let Some(spreadsheet) = parse_spreadsheet(&body) {
                    ...
                    context.set_pin_value("spreadsheet", json!(spreadsheet)).await?;
...
impl NodeLogic for GetGoogleSpreadsheetNode {
    async fn get_node(&self, _app_state: &FlowLikeState) -> Node {
        ...
        node.add_input_pin("provider", "Provider", "Google Drive provider", VariableType::Struct)
            .set_schema::<GoogleProvider>()
            .set_options(PinOptions::new().set_enforce_schema(true).build());
        node.add_input_pin("spreadsheet_id", "Spreadsheet ID", "ID of the spreadsheet", VariableType::String);
        ...
        node.add_required_oauth_scopes(GOOGLE_PROVIDER_ID, vec!["https://www.googleapis.com/auth/spreadsheets.readonly"]);
        node.set_long_running(true);
        node
    }
    ...
    async fn run(&self, context: &mut ExecutionContext) -> flow_like_types::Result<()> {
        ...
        let client = reqwest::Client::new();
        let response = client
            .get(&format!("https://sheets.googleapis.com/v4/spreadsheets/{}", spreadsheet_id))
            .header("Authorization", format!("Bearer {}", provider.access_token))
            .send()
            .await;
...
impl NodeLogic for ReadGoogleSheetsRangeNode {
    async fn get_node(&self, _app_state: &FlowLikeState) -> Node {
        ...
        node.add_input_pin("range", "Range", "A1 notation range (e.g., 'Sheet1!A1:D10')", VariableType::String);
        node.add_input_pin("value_render", "Value Render", "How values should be rendered", VariableType::String)
        ...
        node.add_output_pin("exec_out", "Success", "", VariableType::Execution);
        node.add_output_pin("error", "Error", "", VariableType::Execution);
        node.add_output_pin("values", "Values", "2D array of cell values", VariableType::Generic)
            .set_value_type(ValueType::Array);
    ...
    async fn run(&self, context: &mut ExecutionContext) -> flow_like_types::Result<()> {
        ...
        let value_render: String = context.evaluate_pin("value_render").await.unwrap_or_else(|_| "FORMATTED_VALUE".to_string());
...
impl NodeLogic for WriteGoogleSheetsRangeNode {
    async fn get_node(&self, _app_state: &FlowLikeState) -> Node {
        ...
        node.add_input_pin("values", "Values", "2D array of values to write", VariableType::Generic)
            .set_value_type(ValueType::Array);
        node.add_input_pin("value_input", "Value Input Option", "How input should be interpreted", VariableType::String)
        ...
        node.add_output_pin("exec_out", "Success", "", VariableType::Execution);
        node.add_output_pin("error", "Error", "", VariableType::Execution);
        node.add_output_pin("updated_cells", "Updated Cells", "Number of cells updated", VariableType::Integer);
        node.add_output_pin("error_message", "Error Message", "", VariableType::String);
    ...
    async fn run(&self, context: &mut ExecutionContext) -> flow_like_types::Result<()> {
        ...
        let value_input: String = context.evaluate_pin("value_input").await.unwrap_or_else(|_| "USER_ENTERED".to_string());
        ...
        match response {
            Ok(resp) if resp.status().is_success() => {
                ...
                context.set_pin_value("updated_cells", json!(updated_cells)).await?;
                context.activate_exec_pin("exec_out").await?;
            }
            Ok(resp) => {
                let error = resp.text().await.unwrap_or_default();
...
impl NodeLogic for AppendGoogleSheetsRowsNode {
    async fn get_node(&self, _app_state: &FlowLikeState) -> Node {
        ...
        node.add_input_pin("provider", "Provider", "Google Drive provider", VariableType::Struct)
            ...
            .set_options(PinOptions::new().set_enforce_schema(true).build());
        node.add_input_pin("spreadsheet_id", "Spreadsheet ID", "", VariableType::String);
        node.add_input_pin("range", "Range", "A1 notation range (e.g., 'Sheet1!A:D')", VariableType::String);
        node.add_input_pin("values", "Values", "2D array of row values to append", VariableType::Generic)
            .set_value_type(ValueType::Array);

        node.add_output_pin("exec_out", "Success", "", VariableType::Execution);
        node.add_output_pin("error", "Error", "", VariableType::Execution);
        node.add_output_pin("updated_range", "Updated Range", "Range that was updated", VariableType::String);
        node.add_output_pin("updated_rows", "Updated Rows", "Number of rows appended", VariableType::Integer);
        node.add_output_pin("error_message", "Error Message", "", VariableType::String);
    ...
    async fn run(&self, context: &mut ExecutionContext) -> flow_like_types::Result<()> {
        ...
        match response {
            Ok(resp) if resp.status().is_success() => {
                let body: Value = resp.json().await?;
                let updated_range = body["updates"]["updatedRange"].as_str().unwrap_or("").to_string();
                let updated_rows = body["updates"]["updatedRows"].as_i64().unwrap_or(0);
                context.set_pin_value("updated_range", json!(updated_range)).await?;
                context.set_pin_value("updated_rows", json!(updated_rows)).await?;
                context.activate_exec_pin("exec_out").await?;
            }
            Ok(resp) => {
                let error = resp.text().await.unwrap_or_default();
...
impl NodeLogic for ClearGoogleSheetsRangeNode {
    async fn get_node(&self, _app_state: &FlowLikeState) -> Node {
        ...
        node.add_input_pin("provider", "Provider", "Google Drive provider", VariableType::Struct)
            ...
            .set_options(PinOptions::new().set_enforce_schema(true).build());
        node.add_input_pin("spreadsheet_id", "Spreadsheet ID", "", VariableType::String);
        node.add_input_pin("range", "Range", "A1 notation range to clear", VariableType::String);
    ...
    async fn run(&self, context: &mut ExecutionContext) -> flow_like_types::Result<()> {
        ...
        match response {
            Ok(resp) if resp.status().is_success() => {
                ...
                context.set_pin_value("cleared_range", json!(cleared_range)).await?;
                context.activate_exec_pin("exec_out").await?;
            }
            Ok(resp) => {
                let error = resp.text().await.unwrap_or_default();
...
impl NodeLogic for AddGoogleSheetNode {
    async fn get_node(&self, _app_state: &FlowLikeState) -> Node {
        ...
        node.add_output_pin("exec_out", "Success", "", VariableType::Execution);
        node.add_output_pin("error", "Error", "", VariableType::Execution);
        node.add_output_pin("sheet_id", "Sheet ID", "ID of the new sheet", VariableType::Integer);
        node.add_output_pin("error_message", "Error Message", "", VariableType::String);
...
impl NodeLogic for DeleteGoogleSheetNode {
    async fn get_node(&self, _app_state: &FlowLikeState) -> Node {
        ...
        node.add_input_pin("provider", "Provider", "Google Drive provider", VariableType::Struct)
            ...
            .set_options(PinOptions::new().set_enforce_schema(true).build());
        node.add_input_pin("spreadsheet_id", "Spreadsheet ID", "", VariableType::String);
        node.add_input_pin("sheet_id", "Sheet ID", "ID of sheet to delete", VariableType::Integer);
