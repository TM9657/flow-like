...
impl NodeLogic for CreateGoogleDocNode {
    async fn get_node(&self, _app_state: &FlowLikeState) -> Node {
        ...
        node.add_required_oauth_scopes(GOOGLE_PROVIDER_ID, vec!["https://www.googleapis.com/auth/documents"]);
...
impl NodeLogic for GetGoogleDocNode {
    async fn get_node(&self, _app_state: &FlowLikeState) -> Node {
        ...
        node.add_output_pin("document", "Document", "", VariableType::Struct)
            .set_schema::<GoogleDocument>();
        node.add_output_pin("content", "Content", "Raw document content JSON", VariableType::Generic);
        node.add_output_pin("error_message", "Error Message", "", VariableType::String);

        node.add_required_oauth_scopes(GOOGLE_PROVIDER_ID, vec!["https://www.googleapis.com/auth/documents.readonly"]);
        node.set_long_running(true);
        node
    }
    ...
    async fn run(&self, context: &mut ExecutionContext) -> flow_like_types::Result<()> {
        ...
        let client = reqwest::Client::new();
        let response = client
            .get(&format!("https://docs.googleapis.com/v1/documents/{}", document_id))
            .header("Authorization", format!("Bearer {}", provider.access_token))
            .send()
            .await;
...
impl NodeLogic for InsertGoogleDocTextNode {
    async fn get_node(&self, _app_state: &FlowLikeState) -> Node {
        ...
        node.add_input_pin("index", "Index", "Character index to insert at (1 = start)", VariableType::Integer)
    ...
    async fn run(&self, context: &mut ExecutionContext) -> flow_like_types::Result<()> {
        ...
        let client = reqwest::Client::new();
        let response = client
            .post(&format!("https://docs.googleapis.com/v1/documents/{}:batchUpdate", document_id))
            .header("Authorization", format!("Bearer {}", provider.access_token))
            .header("Content-Type", "application/json")
            .json(&body)
            .send()
...
impl NodeLogic for DeleteGoogleDocTextNode {
    async fn get_node(&self, _app_state: &FlowLikeState) -> Node {
        ...
        node.add_input_pin("provider", "Provider", "Google Drive provider", VariableType::Struct)
            ...
            .set_options(PinOptions::new().set_enforce_schema(true).build());
        node.add_input_pin("document_id", "Document ID", "", VariableType::String);
        node.add_input_pin("start_index", "Start Index", "Start index (inclusive)", VariableType::Integer);
        node.add_input_pin("end_index", "End Index", "End index (exclusive)", VariableType::Integer);
...
impl NodeLogic for ReplaceGoogleDocTextNode {
    async fn get_node(&self, _app_state: &FlowLikeState) -> Node {
        ...
        node.add_input_pin("provider", "Provider", "Google Drive provider", VariableType::Struct)
            ...
            .set_options(PinOptions::new().set_enforce_schema(true).build());
        node.add_input_pin("document_id", "Document ID", "", VariableType::String);
        node.add_input_pin("search_text", "Search Text", "Text to find", VariableType::String);
        node.add_input_pin("replace_text", "Replace Text", "Replacement text", VariableType::String);
        node.add_input_pin("match_case", "Match Case", "Case-sensitive search", VariableType::Boolean)
            .set_default_value(Some(json!(true)));

        node.add_output_pin("exec_out", "Success", "", VariableType::Execution);
        node.add_output_pin("error", "Error", "", VariableType::Execution);
        node.add_output_pin("occurrences", "Occurrences", "Number of replacements", VariableType::Integer);
        node.add_output_pin("error_message", "Error Message", "", VariableType::String);
        ...
        match response {
            Ok(resp) if resp.status().is_success() => {
                ...
                let occurrences = body["replies"][0]["replaceAllText"]["occurrencesChanged"]
                    .as_i64()
                    .unwrap_or(0);
                context.set_pin_value("occurrences", json!(occurrences)).await?;
                context.activate_exec_pin("exec_out").await?;
            }
            Ok(resp) => {
                let error = resp.text().await.unwrap_or_default();
...
impl NodeLogic for ExportGoogleDocNode {
    async fn get_node(&self, _app_state: &FlowLikeState) -> Node {
        ...
        node.add_output_pin("exec_out", "Success", "", VariableType::Execution);
        node.add_output_pin("error", "Error", "", VariableType::Execution);
        node.add_output_pin("content", "Content", "Exported content (string for text formats, base64 for binary)", VariableType::String);
        node.add_output_pin("error_message", "Error Message", "", VariableType::String);
    ...
    async fn run(&self, context: &mut ExecutionContext) -> flow_like_types::Result<()> {
        ...
        let provider: GoogleProvider = context.evaluate_pin("provider").await?;
        let document_id: String = context.evaluate_pin("document_id").await?;
        let format: String = context.evaluate_pin("format").await.unwrap_or_else(|_| "text/plain".to_string());
