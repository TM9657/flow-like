...
use flow_like_types::{async_trait, create_id, json::json, reqwest, JsonSchema, Value};
...
impl NodeLogic for CreateGoogleSlidesNode {
    async fn get_node(&self, _app_state: &FlowLikeState) -> Node {
        ...
        node.add_output_pin("presentation_id", "Presentation ID", "", VariableType::String);
        ...
        node.add_required_oauth_scopes(GOOGLE_PROVIDER_ID, vec!["https://www.googleapis.com/auth/presentations"]);
        ...
        match response {
            Ok(resp) if resp.status().is_success() => {
                ...
                if let Some(presentation) = parse_presentation(&body) {
                    ...
                    context.set_pin_value("presentation", json!(presentation)).await?;
...
impl NodeLogic for GetGoogleSlidesNode {
    async fn get_node(&self, _app_state: &FlowLikeState) -> Node {
        ...
        node.add_input_pin("provider", "Provider", "Google Drive provider", VariableType::Struct)
            .set_schema::<GoogleProvider>()
            .set_options(PinOptions::new().set_enforce_schema(true).build());
        node.add_input_pin("presentation_id", "Presentation ID", "", VariableType::String);
        ...
        node.add_required_oauth_scopes(GOOGLE_PROVIDER_ID, vec!["https://www.googleapis.com/auth/presentations.readonly"]);
        node.set_long_running(true);
        node
    }
    ...
    async fn run(&self, context: &mut ExecutionContext) -> flow_like_types::Result<()> {
        ...
        let client = reqwest::Client::new();
        let response = client
            .get(&format!("https://slides.googleapis.com/v1/presentations/{}", presentation_id))
            .header("Authorization", format!("Bearer {}", provider.access_token))
            .send()
            .await;
        ...
        match response {
            Ok(resp) if resp.status().is_success() => {
                ...
                if let Some(presentation) = parse_presentation(&body) {
                    ...
                    context.set_pin_value("slide_count", json!(slide_count)).await?;
                    context.activate_exec_pin("exec_out").await?;
                } else {
                    context.set_pin_value("error_message", json!("Failed to parse response")).await?;
                    context.activate_exec_pin("error").await?;
...
impl NodeLogic for AddGoogleSlideNode {
    async fn get_node(&self, _app_state: &FlowLikeState) -> Node {
        ...
        node.add_input_pin("provider", "Provider", "Google Drive provider", VariableType::Struct)
            ...
            .set_options(PinOptions::new().set_enforce_schema(true).build());
        node.add_input_pin("presentation_id", "Presentation ID", "", VariableType::String);
        node.add_input_pin("layout", "Layout", "Predefined layout for the slide", VariableType::String)
        ...
        node.add_input_pin("insert_index", "Insert Index", "Index where to insert slide (optional)", VariableType::Integer)
            .set_default_value(Some(json!(-1)));

        node.add_output_pin("exec_out", "Success", "", VariableType::Execution);
        node.add_output_pin("error", "Error", "", VariableType::Execution);
        node.add_output_pin("slide_id", "Slide ID", "ID of the created slide", VariableType::String);
        node.add_output_pin("error_message", "Error Message", "", VariableType::String);
    ...
    async fn run(&self, context: &mut ExecutionContext) -> flow_like_types::Result<()> {
        ...
        let provider: GoogleProvider = context.evaluate_pin("provider").await?;
        let presentation_id: String = context.evaluate_pin("presentation_id").await?;
        let layout: String = context.evaluate_pin("layout").await.unwrap_or_else(|_| "BLANK".to_string());
        ...
        let client = reqwest::Client::new();
        let response = client
            .post(&format!("https://slides.googleapis.com/v1/presentations/{}:batchUpdate", presentation_id))
            .header("Authorization", format!("Bearer {}", provider.access_token))
            .header("Content-Type", "application/json")
            .json(&body)
            .send()
...
impl NodeLogic for DeleteGoogleSlideNode {
    async fn get_node(&self, _app_state: &FlowLikeState) -> Node {
        ...
        node.add_input_pin("provider", "Provider", "Google Drive provider", VariableType::Struct)
            ...
            .set_options(PinOptions::new().set_enforce_schema(true).build());
        node.add_input_pin("presentation_id", "Presentation ID", "", VariableType::String);
        node.add_input_pin("slide_id", "Slide ID", "ID of the slide to delete", VariableType::String);
...
impl NodeLogic for AddTextToSlideNode {
    async fn get_node(&self, _app_state: &FlowLikeState) -> Node {
        ...
        node.add_input_pin("provider", "Provider", "Google Drive provider", VariableType::Struct)
            ...
            .set_options(PinOptions::new().set_enforce_schema(true).build());
        node.add_input_pin("presentation_id", "Presentation ID", "", VariableType::String);
        node.add_input_pin("slide_id", "Slide ID", "ID of the slide", VariableType::String);
        node.add_input_pin("text", "Text", "Text content to add", VariableType::String);
        node.add_input_pin("x", "X Position", "X position in EMU (914400 EMU = 1 inch)", VariableType::Float)
        ...
        node.add_output_pin("exec_out", "Success", "", VariableType::Execution);
        node.add_output_pin("error", "Error", "", VariableType::Execution);
        node.add_output_pin("shape_id", "Shape ID", "ID of the created text box", VariableType::String);
        node.add_output_pin("error_message", "Error Message", "", VariableType::String);
...
impl NodeLogic for ExportGoogleSlidesNode {
    async fn get_node(&self, _app_state: &FlowLikeState) -> Node {
        ...
        node.add_output_pin("exec_out", "Success", "", VariableType::Execution);
        node.add_output_pin("error", "Error", "", VariableType::Execution);
        node.add_output_pin("content", "Content", "Exported content (base64 encoded for binary formats)", VariableType::String);
        node.add_output_pin("error_message", "Error Message", "", VariableType::String);
    ...
    async fn run(&self, context: &mut ExecutionContext) -> flow_like_types::Result<()> {
        ...
        let provider: GoogleProvider = context.evaluate_pin("provider").await?;
        let presentation_id: String = context.evaluate_pin("presentation_id").await?;
        let format: String = context.evaluate_pin("format").await.unwrap_or_else(|_| "application/pdf".to_string());
