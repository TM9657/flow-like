...
// =============================================================================
// Copilot Retrieval Node (Official Copilot Retrieval API - Preview)
// Based on Microsoft Agents M365 Copilot SDK
// https://github.com/microsoft/Agents-M365Copilot
// =============================================================================

/// Entity type returned from retrieval API
/// Based on RetrievalEntityType enum in official SDK
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "camelCase")]
pub enum CopilotRetrievalEntityType {
    Site,
    List,
    ListItem,
    Drive,
...
/// Data source for retrieval API
/// Based on RetrievalDataSource enum in official SDK
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
#[serde(rename_all = "camelCase")]
pub enum CopilotRetrievalDataSource {
    SharePoint,
    OneDriveBusiness,
    ExternalItem,
    UnknownFutureValue,
...
/// Sensitivity label information for retrieved content
/// Based on sensitivityLabel from official Microsoft 365 Copilot Retrieval API
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
pub struct CopilotSensitivityLabelInfo {
    pub sensitivity_label_id: Option<String>,
    pub display_name: Option<String>,
...
/// Extract from retrieved content with relevance score
/// Based on RetrievalExtract model in official SDK
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
pub struct CopilotRetrievalExtract {
    pub text: String,
    pub relevance_score: Option<f64>,
}

/// Resource metadata dictionary from retrieval API
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
pub struct CopilotResourceMetadata {
    /// Additional metadata fields as key-value pairs
    #[serde(flatten)]
    pub fields: std::collections::HashMap<String, Value>,
}

/// A single retrieval hit from Copilot API
/// Based on RetrievalHit model in official SDK
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
pub struct CopilotRetrievalHit {
    pub web_url: Option<String>,
    pub resource_type: Option<String>,
    pub extracts: Vec<CopilotRetrievalExtract>,
    pub resource_metadata: Option<CopilotResourceMetadata>,
    pub sensitivity_label: Option<CopilotSensitivityLabelInfo>,
}

/// Response from Copilot Retrieval API
/// Based on RetrievalResponse model in official SDK
#[derive(Debug, Clone, Serialize, Deserialize, JsonSchema)]
pub struct CopilotRetrievalResponse {
    pub retrieval_hits: Vec<CopilotRetrievalHit>,
}

fn parse_sensitivity_label(value: &Value) -> Option<CopilotSensitivityLabelInfo> {
    if value.is_null() {
        return None;
    }
    Some(CopilotSensitivityLabelInfo {
        sensitivity_label_id: value["sensitivityLabelId"].as_str().map(String::from),
        display_name: value["displayName"].as_str().map(String::from),
        tool_tip: value["toolTip"].as_str().map(String::from),
        priority: value["priority"].as_i64().map(|p| p as i32),
        color: value["color"].as_str().map(String::from),
    })
}

fn parse_resource_metadata(value: &Value) -> Option<CopilotResourceMetadata> {
    ...
    let fields = value
        ...
        .unwrap_or_default();
    Some(CopilotResourceMetadata { fields })
}

fn parse_retrieval_extract(value: &Value) -> Option<CopilotRetrievalExtract> {
    Some(CopilotRetrievalExtract {
        text: value["text"].as_str()?.to_string(),
        relevance_score: value["relevanceScore"].as_f64(),
    })
}

fn parse_retrieval_hit(value: &Value) -> Option<CopilotRetrievalHit> {
    let extracts = value["extracts"]
        .as_array()
        .map(|arr| arr.iter().filter_map(parse_retrieval_extract).collect())
        .unwrap_or_default();

    Some(CopilotRetrievalHit {
        web_url: value["webUrl"].as_str().map(String::from),
        resource_type: value["resourceType"].as_str().map(String::from),
        extracts,
        resource_metadata: parse_resource_metadata(&value["resourceMetadata"]),
        sensitivity_label: parse_sensitivity_label(&value["sensitivityLabel"]),
    })
}

#[crate::register_node]
#[derive(Default)]
pub struct CopilotRetrievalNode {}

impl CopilotRetrievalNode {
    pub fn new() -> Self {
        Self {}
    }
}
...
#[async_trait]
impl NodeLogic for CopilotRetrievalNode {
    async fn get_node(&self, _app_state: &FlowLikeState) -> Node {
        let mut node = Node::new(
            "data_microsoft_copilot_retrieval",
            "Copilot Retrieval (Preview)",
            "Retrieve relevant content from SharePoint/OneDrive using Microsoft 365 Copilot's semantic retrieval API. This is the official Copilot API for content discovery.",
            "Data/Microsoft/Copilot",
        );
        node.add_icon("/flow/icons/copilot.svg");
        ...
        node.add_input_pin(
            "query",
            "Query",
            "Natural language query for retrieval",
            VariableType::String,
        );
        node.add_input_pin(
            "data_source",
        ...
        .set_default_value(Some(json!("sharePoint")))
        .set_options(
            PinOptions::new()
                .set_valid_values(vec![
                    "sharePoint".to_string(),
                    "oneDriveBusiness".to_string(),
                    "externalItem".to_string(),
                ])
                .build(),
        );
        node.add_input_pin(
        ...
        node.add_input_pin(
            "resource_metadata",
            "Resource Metadata",
            "Optional comma-separated list of metadata fields to include in response",
            VariableType::String,
        )
        .set_default_value(Some(json!("")));
        ...
        .set_schema::<CopilotRetrievalResponse>();
        node.add_output_pin("hits", "Hits", "Retrieval hits", VariableType::Struct)
            .set_value_type(ValueType::Array)
            .set_schema::<CopilotRetrievalHit>();
    ...
    async fn run(&self, context: &mut ExecutionContext) -> flow_like_types::Result<()> {
        ...
        let provider: MicrosoftGraphProvider = context.evaluate_pin("provider").await?;
        let query: String = context.evaluate_pin("query").await?;
        let data_source: String = context
            .evaluate_pin("data_source")
            .await
            .unwrap_or_else(|_| "sharePoint".to_string());
        let max_results: i64 = context.evaluate_pin("max_results").await.unwrap_or(10);
        let filter_expression: String = context
            .evaluate_pin("filter_expression")
            .await
            .unwrap_or_default();
        ...
        if !filter_expression.is_empty() {
            request_body["filterExpression"] = json!(filter_expression);
        }
        ...
        if !resource_metadata.is_empty() {
            let metadata_fields: Vec<String> = resource_metadata
                ...
                .collect();
            if !metadata_fields.is_empty() {
                request_body["resourceMetadata"] = json!(metadata_fields);
            }
        }
        ...
        match response {
            Ok(resp) if resp.status().is_success() => {
                let body: Value = resp.json().await?;
                let mut hits: Vec<CopilotRetrievalHit> = Vec::new();

                if let Some(retrieval_hits) = body["retrievalHits"].as_array() {
                    for hit in retrieval_hits {
                        if let Some(parsed_hit) = parse_retrieval_hit(hit) {
                            hits.push(parsed_hit);
                        }
                    }
                }

                let count = hits.len() as i64;
                let response = CopilotRetrievalResponse {
                    retrieval_hits: hits.clone(),
                };

                context.set_pin_value("response", json!(response)).await?;
                context.set_pin_value("hits", json!(hits)).await?;
                context.set_pin_value("count", json!(count)).await?;
                context.activate_exec_pin("exec_out").await?;
            }
            Ok(resp) => {
