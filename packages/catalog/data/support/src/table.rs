...
impl CSVTable {
    ...
    /// Borrow column names without allocating copies.
    pub fn headers_ref(&self) -> &[Arc<str>] {
        &self.headers
    }

    /// Borrow the typed cells in their original row order.
    pub fn rows(&self) -> &[Box<[Cell]>] {
        &self.rows
    }
