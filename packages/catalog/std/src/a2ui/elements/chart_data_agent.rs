...
fn get_chart_format_description(chart_type: &str) -> &'static str {
    match chart_type {
        "bar" => r#"Bar chart format: Array of objects where each object is a category with numeric values.
...
Output includes: data (array), keys (value field names), indexBy (category field name)"#,
        ...
        "line" => r#"Line chart format: Array of series objects, each with id and data array of {x, y} points.
Example: [{ "id": "Revenue", "data": [{"x": "Jan", "y": 100}, {"x": "Feb", "y": 120}] }]
X values can be dates, categories, or numbers. Y values must be numeric."#,

        "pie" => r#"Pie/Donut chart format: Array of slice objects with id, label, and value.
...
Values should sum to a meaningful total (percentages or counts)."#,
        ...
        "radar" => r#"Radar/Spider chart format: Array of dimension objects with values for each series.
...
Output includes: data (array), keys (series names), indexBy (dimension field name)"#,
        ...
        "heatmap" => r#"Heatmap format: Array of row objects, each with id and data array of {x, y} cells.
Example: [{ "id": "Monday", "data": [{"x": "9am", "y": 5}, {"x": "10am", "y": 12}] }]
Y values represent the heat/intensity at each cell."#,

        "sankey" => r#"Sankey diagram format: Object with nodes and links arrays.
...
Links represent flow from source to target with a value/weight."#,
        ...
        "calendar" => r#"Calendar heatmap format: Array of day/value pairs with YYYY-MM-DD dates.
Example: [{ "day": "2024-01-15", "value": 50 }, { "day": "2024-01-16", "value": 30 }]
Output includes: data (array), from_date, to_date (date range strings)"#,

        "treemap" => r#"Treemap/Sunburst hierarchical format: Nested object with name and children/value.
...
Leaf nodes have value, branch nodes have children array."#,
        ...
        "scatter" => r#"Scatter plot format: Array of series with data points containing x and y coordinates.
Example: [{ "id": "Group A", "data": [{"x": 10, "y": 20}, {"x": 15, "y": 30}] }]
Both x and y should be numeric for scatter plots."#,

        "funnel" => r#"Funnel chart format: Array of stage objects with id, label, and value (ordered by value descending).
...
Values typically decrease through the funnel stages."#,
        ...
        "chord" => r#"Chord diagram format: Square matrix of relationships between entities.
...
impl NodeLogic for ChartDataAgent {
    fn get_node(&self) -> Node {
        ...
        node.add_output_pin("data", "Data", "Chart-ready data in the appropriate format", VariableType::Generic);
        node.add_output_pin("sql", "SQL", "The generated SQL query", VariableType::String);
        node.add_output_pin("keys", "Keys", "Series/value keys for the chart (if applicable)", VariableType::Generic);
        node.add_output_pin("index_by", "Index By", "Index/category field name (if applicable)", VariableType::String);
        node.add_output_pin("explanation", "Explanation", "AI explanation of the query and data", VariableType::String);
    ...
    async fn run(&self, context: &mut ExecutionContext) -> flow_like_types::Result<()> {
        ...
        let table_schemas: Value = context.evaluate_pin("table_schemas").await.unwrap_or(json!({}));

        let table_schemas_str = if table_schemas.is_null() || table_schemas.as_object().map(|o| o.is_empty()).unwrap_or(true) {
        ...
        context.set_pin_value("sql", json!("-- SQL generation requires LLM integration")).await?;
        context.set_pin_value("keys", json!([])).await?;
        context.set_pin_value("index_by", json!("")).await?;
        context.set_pin_value("explanation", json!(format!(
