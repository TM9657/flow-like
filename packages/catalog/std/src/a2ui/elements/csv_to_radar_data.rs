...
impl NodeLogic for CsvToRadarData {
    fn get_node(&self) -> Node {
        ...
        node.add_output_pin("data", "Data", "Radar chart data array", VariableType::Generic);
        node.add_output_pin("keys", "Keys", "Series keys for the chart", VariableType::Generic);
        node.add_output_pin("index_by", "Index By", "Dimension field name", VariableType::String);
    ...
    async fn run(&self, context: &mut ExecutionContext) -> flow_like_types::Result<()> {
        ...
        if headers.is_empty() || rows.is_empty() {
            ...
            context.set_pin_value("index_by", json!("dimension")).await?;
            return Ok(());
        }

        let dim_idx = parse_column_ref(&dim_col, &headers);
        let index_field = clean_field_name(headers.get(dim_idx).map(|s| s.as_str()).unwrap_or("dimension"));

        let series_indices: Vec<usize> = if series_cols.is_empty() {
            (0..headers.len()).filter(|&i| i != dim_idx).collect()
        } else {
            series_cols.split(',').map(|s| parse_column_ref(s.trim(), &headers)).filter(|&i| i != dim_idx && i < headers.len()).collect()
        };

        let keys: Vec<String> = series_indices.iter().map(|&i| clean_field_name(headers.get(i).map(|s| s.as_str()).unwrap_or(&format!("series{}", i)))).collect();

        let data: Vec<Value> = rows.iter().map(|row| {
            let mut obj = Map::new();
            obj.insert(index_field.clone(), json!(row.get(dim_idx).cloned().unwrap_or_default()));
