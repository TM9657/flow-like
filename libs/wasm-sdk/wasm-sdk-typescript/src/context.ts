...
export class Context {
  ...
  // Metadata getters
  get nodeId(): string { return this._input.nodeId; }
  get nodeName(): string { return this._input.nodeName; }
  get runId(): string { return this._input.runId; }
  get appId(): string { return this._input.appId; }
  get boardId(): string { return this._input.boardId; }
  get userId(): string { return this._input.userId; }
  get streamEnabled(): boolean { return this._input.streamState; }
  get logLevel(): number { return this._input.logLevel; }

  // Input getters
  getInput(name: string): unknown { return this._input.inputs[name] ?? null; }
  ...
  debug(message: string): void {
    if (this._input.logLevel <= LogLevel.DEBUG) this._host.log(LogLevel.DEBUG, message);
  }

  info(message: string): void {
    if (this._input.logLevel <= LogLevel.INFO) this._host.log(LogLevel.INFO, message);
  }

  warn(message: string): void {
    if (this._input.logLevel <= LogLevel.WARN) this._host.log(LogLevel.WARN, message);
  }

  error(message: string): void {
    if (this._input.logLevel <= LogLevel.ERROR) this._host.log(LogLevel.ERROR, message);
  }
  ...
  streamJSON(data: unknown): void {
    if (this._input.streamState) this._host.stream("json", JSON.stringify(data));
  }
  ...
  httpRequest(method: number, url: string, headers: Record<string, string> = {}, body: Uint8Array | null = null): { status: number; headers: Record<string, string>; body: string } | null {
    const result = this._host.httpRequest(method, url, JSON.stringify(headers), body);
    if (!result) return null;
    return JSON.parse(result);
  }

  httpGet(url: string, headers: Record<string, string> = {}): { status: number; headers: Record<string, string>; body: string } | null {
    return this.httpRequest(0, url, headers);
  }

  httpPost(url: string, body: Uint8Array | null = null, headers: Record<string, string> = {}): { status: number; headers: Record<string, string>; body: string } | null {
